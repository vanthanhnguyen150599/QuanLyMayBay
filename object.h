...
int InDanhSachHanhKhachThuocChuyenBay(ListChuyenBay &a, string &MaChuyenBay)
{
	...
	char ngatu = 197;
	string *MaChuyen;
	int dem = SoLuongChuyenBayHuyVe(a);
	if (dem != 0)
	{
		MaChuyen = new string[dem];
		LayDanhSachHuyVe(a,MaChuyen);
	}
	else
	{
		gotoxy(0,30);
			...
			case 5: // xem
			{
				...
				//	KhungChuyenBayDenX();
					...
					for (CacChuyenBay *p = a.Head; p != NULL; p = p->next)
					{
						...
						{
						//	Mang[SoLuong] = new CacChuyenBay;
							Mang[SoLuong] = p;
						//	InGio(p->chuyenbay.LayGio()); cout << " " << Mang[SoLuong]->chuyenbay.LayGio().hour;
							SoLuong++;
						}
					}
					if (SoLuong == 0)
